...
#define MAXPATCH           600   // max number patches. Note that this includes the "patches" created as a result of discards steps 
...
Rprintf("Start of DynStateF\n");
  ...
  /********************************************************************************************************************/
  /* Checks for modelsizes HORIZON,, MAXPATCH, MAXNOINC if so simulations will not extend arrays                      */
  /********************************************************************************************************************/
  if (kHorizon > MAXHORIZON){ Rprintf("Warning, horizon dimensions exceed array \n");} else { Rprintf("Horizon dimensions not exceeding array, OK \n");} 
  if (noInc > MAXNOINC) {Rprintf("Warning, noinc larger than MAXNOINC \n");} else { Rprintf("Noinc smaller than MAXNOINC, OK \n");}
  if (kNPatch > MAXPATCH ) {Rprintf("Warning, number of patches (including those generated by discarding) larger than MAXPATCH \n");} else { Rprintf("Number of patches smaller than MAXPATCH, OK \n");}
  /********************************************************************************************************************/
	            ...
	            for (inc3 = 0; inc3 < noInc; inc3++){
	              for (inc4 = 0; inc4 < noInc; inc4++){
	                for (inc5 = 0; inc5 < noInc; inc5++){
		                    theLndParmsAgg[i][t][s][inc0 + inc1 + inc2 + inc3 + inc4 + inc5] +=  theLndParms[i][t][s][0][inc0] * 
