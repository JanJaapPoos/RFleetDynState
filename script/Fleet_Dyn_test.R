...
# Catches mac
catchMean(mac)  <- array(5, dim=c(6,4,2), dimnames=dimnames(catchMean(hke)))
catchSigma(mac) <- array(1 , dim=c(6,4,2), dimnames=dimnames(catchMean(hke)))

# Other catches
catchMean(mon)  <- catchMean(hom)  <- catchMean(meg)  <- array(0  , dim=c(6,4,2), dimnames=dimnames(catchMean(hke)))
catchSigma(mon) <- catchSigma(hom) <- catchSigma(meg) <- array(0.1, dim=c(6,4,2), dimnames=dimnames(catchMean(hke)))
...
macPrice <- array(5,                   dim=c(6,4), dimnames = dimnames(hkePrice))
monPrice <- homPrice <- megPrice <- macPrice <- array(0, dim=c(6,4), dimnames = dimnames(hkePrice))

##############################################################################
# test 1 unconstrained hake quotum, high hake price
###############################################################################
control <- DynState.control(spp1LndQuota= 600,  spp2LndQuota= 600,#tn
...
##############################################################################
# test 2 constrained hake quotum, high hake price
###############################################################################
control@spp1LndQuota <- 480
...
##############################################################################
# test 2 constrained hake quotum, high hake price, no discards allowed
###############################################################################
